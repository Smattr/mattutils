...
///
/// In a large interactive Git rebase, it is common to find yourself staring at
/// short commit messages, trying to recall what the contents of that commit
/// actually were. I find myself often suspending the picklist to run various
/// `git show` commands to inspect commits. It's possible to tweak
/// rebase.intructionFormat to give you a slightly more informative line, but
/// things can still be quite cryptic. To ease this pain, this tool wraps Vim to
/// act as your GIT_EDITOR. It retrieves diffs for each commit in the picklist,
/// appending them as folded comments to the commit line. So instead of getting
/// a picklist like:
///
...
///     ...
///
/// To use this, you probably want to configure a Git alias for it. Something
/// like:
///
///     [alias]
...
static int insert_diffs(FILE *in, FILE *out) {
  ...
  for (;;) {
    ...
    if (regexec(&pick, line, sizeof match / sizeof match[0], match, 0) == 0) {
      ...
      char *command;
      if (asprintf(&command, "git show %.*s",
...
int main(int argc, char **argv) {
  ...
  // Now open Vim with the new file with all folds collapsed
  {
    char *command;
    if (asprintf(&command,
                 "vim '+set foldmethod=marker' '+set foldlevel=0' '%s'",
                 argv[1]) < 0) {
  ...
  // Now strip the comment lines. This is not strictly necessary, but Git’s
  // interactive rebase script seems incredibly slow to pass over long stretches
  // of comment lines.
