 ...
 * a599fa4faa958c44ab270e1a418abc8953343e2d.
 *
 * To run a Murphi model, you can use a tool like Rumur. Constructing this model
 * actually uncovered a bug in Rumur which was only subsequently fixed. So you
 * will need a recent Rumur version to successfully run this model.
...
const LOAD_SCALE: N_THREAD + 1;

const REFS_MASK: LOAD_SCALE - 1;

-- type of a counter value
type count_t: 0..N_THREAD + N_THREAD * LOAD_SCALE;
...
type pc_t: enum {
  SP_ACQ_L1,   -- sp_acq, asp.c:223
  SP_ACQ_L2,   -- sp_acq, asp.c:242
  SP_ACQ_L3,   -- sp_acq, asp.c:243
  SP_ACQ_L4,   -- sp_acq, asp.c:247
  SP_ACQ_L5,   -- sp_acq, asp.c:254
  SP_REL_L1,   -- dec_ref, asp.c:147
  SP_STORE_L1, -- sp_store, asp.c:292
  SP_STORE_L2, -- sp_store, asp.c:298
  SP_STORE_L3  -- dec_ref, asp.c:147
};
...
begin
  ...
  sp_ctrls[ctrl].ref_count := sp_ctrls[ctrl].ref_count + by_ * LOAD_SCALE;
end;
...
begin
  ...
  assert (sp_ctrls[ctrl].ref_count & REFS_MASK) > 0
    "changing load count while not holding a reference";
  sp_ctrls[ctrl].ref_count := sp_ctrls[ctrl].ref_count - LOAD_SCALE;
end;
...
ruleset tid: tid_t do
  ...
  begin
    ...
    assert tls[tid].sp_acq.new.load_count < LOAD_SCALE "future overflow";
