...
static inline bool set_slot_is_free(uintptr_t slot) {
  return slot == 0;
}

/// does this set slot contain an item that was deleted?
static inline bool set_slot_is_deleted(uintptr_t slot) {
  return slot & 1;
}

/// convert a set slot to its originating item pointer
static inline void *set_slot_to_ptr(uintptr_t slot) {
