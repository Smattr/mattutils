 ...
 * 1721fea2cb3fa569e2513c4e59f5c2270b94a0fa.
 *
...
type count_t: 0..N_THREAD + 1;
type icount_t: -N_THREAD..N_THREAD;
...
type pc_t: enum {
  SP_ACQ_L1,   -- sp_acq, asp.c:274
  SP_ACQ_L2,   -- sp_acq, asp.c:291
  SP_ACQ_L3,   -- sp_acq, asp.c:292
  SP_ACQ_L4,   -- sp_acq, asp.c:297
  SP_ACQ_L5,   -- sp_acq, asp.c:303
  SP_REL_L1,   -- dec_ref, asp.c:192
  SP_STORE_L1, -- sp_store, asp.c:341
  SP_STORE_L2, -- sp_store, asp.c:347
  SP_STORE_L3  -- dec_ref, asp.c:192
};
...
begin
  for i: sp_ctrl_ptr_t do
    if !sp_ctrls_allocated[i] then
      ...
      sp_ctrls[i].rc.refs := 0;
      sp_ctrls[i].rc.loads := 0;
      return i;
    end;
  end;
  return 0;
...
begin
  ...
  -- for simplicity, the CAS loop here is not modelled
  sp_ctrls[ctrl].rc.refs := sp_ctrls[ctrl].rc.refs + by_;
end;

function inc_load_ref(ctrl: sp_ctrl_ptr_t; by_: count_t);
begin
  assert !isundefined(ctrl);
  assert sp_ctrls[ctrl].rc.refs > 0
    "changing load count while not holding a reference";
  -- for simplicity, the CAS loop here is not modelled
  sp_ctrls[ctrl].rc.loads := sp_ctrls[ctrl].rc.loads + by_;
end;

function dec_ref_1(ctrl: sp_ctrl_ptr_t): rc_t;
  var old: rc_t;
begin
  assert !isundefined(ctrl);
  old := sp_ctrls[ctrl].rc;
  -- for simplicity, the CAS loop here is not modelled
  sp_ctrls[ctrl].rc.refs := sp_ctrls[ctrl].rc.refs - 1;
  assert old.refs > 0 "dropping a reference that was not held";
  return old;
end;

function dec_ref_2(ctrl: sp_ctrl_ptr_t; old: rc_t);
begin
  assert !isundefined(ctrl);
  -- “if we just dropped the last reference, clean up”
  if old.refs = 1 & old.loads = 0 then
    free_sp_ctrl(ctrl);
  end;
end;
...
begin
  ...
  assert sp_ctrls[ctrl].rc.refs > 0
    "changing load count while not holding a reference";
  -- for simplicity, the CAS loop here is not modelled
  sp_ctrls[ctrl].rc.loads := sp_ctrls[ctrl].rc.loads - 1;
end;
