...
bool uint128_atomic_cas(uint128_t *dst, uint128_t *expected,
                        uint128_t desired) {
  assert(dst != NULL);
  ...
  const uint128_t expectation = *expected;
  const uint128_t old = __sync_val_compare_and_swap(dst, expectation, desired);
  if (old == expectation)
    return true;
  *expected = old;
  return false;
