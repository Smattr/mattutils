/// @file
/// @brief Test multi-threaded atomic exchange of a 128-bit unsigned integer
///
...
static THREAD_RET entry(void *arg) {
  ...
  const uint128_t old = uint128_atomic_xchg(s->target, desired);

  // we should have seen either the initial value or something written by a
  // thread that is not ourselves
  const bool ok = old != s->thread_id + 1 && old <= s->n_threads;
  ASSERT(ok);

  return 0;
}

TEST("uint128_atomic_xchg multi-threaded") {
