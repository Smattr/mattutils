/// @file
/// @brief Test multi-threaded atomic CAS of a 128-bit unsigned integer
///
...
static THREAD_RET entry(void *arg) {
  ...
  if (s->thread_id == 0) {
    uint128_t expected = s->init_val;
    const bool r = uint128_atomic_cas(s->target, &expected, s->thread_id);
  ...
  while (true) {
    uint128_t expected = s->thread_id - 1;
    if (uint128_atomic_cas(s->target, &expected, s->thread_id))
      break;
  }
...
TEST("uint128_atomic_cas multi-threaded") {
