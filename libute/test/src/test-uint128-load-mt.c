/// @file
/// @brief Test multi-threaded atomic reading of a 128-bit unsigned integer
///
...
static void run(uint128_t *target) {
  assert(target != NULL);
...
TEST("uint128_atomic_load multi-threaded") {
  run(&(uint128_t){0});
  run(&(uint128_t){1});
  run(&(uint128_t){42});
  run(&(uint128_t){(uint128_t)UINT64_MAX + 1});
}
