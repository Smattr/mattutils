#!/usr/bin/env bash

# This script symlinks all relevant scripts from this repository into ~/bin and
# configuration settings into your home directory.

# Make a failure terminate the script
set -e

REPO="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && cd .. && pwd )"

mkdir -p "${HOME}/bin"

# Link useful scripts.
for i in \
         addcert.sh \
         after \
         ag \
         bashd.sh \
         check_tmux_version.sh \
         cmake \
         compresspdf.sh \
         fifi.sh \
         find-broken.sh \
         fwdmail.py \
         generate-passwd \
         g \
         ifind \
         kage \
         timestamp \
         toggle-screensaver \
         term \
         meta_c \
         meta_h \
         meta_l \
         meta_u \
         meta_z \
         notate.py \
         package.py \
         pdfcrop.sh \
         prefix \
         pyman \
         search \
         ssh-proxy \
         t \
         ubiquity \
         validate.py \
         wim ; do
    if [ ! -e "${REPO}/$i" ]; then
        echo "Error: $i not found in working directory." >&2
        exit 1
    fi
    if [ ! -e "${HOME}/bin/$i" ]; then
        ln -s "${REPO}/$i" "${HOME}/bin/$i"
    elif [ ! -L "${HOME}/bin/$i" ]; then
        echo "Warning: Skipping $i that already exists." >&2
    fi
done
SHELL_PATH=$(${SHELL} -c 'echo ${PATH}')
if [[ ":${SHELL_PATH}:" != *":${HOME}/bin:"* ]]; then
    echo "Warning: ${HOME}/bin, where symlinks are created, is not in your bash \$PATH." >&2
fi

# Link configurations.
for i in .agignore \
         .dircolors \
         .emacs \
         .fonts.conf \
         .gdbinit \
         .gitconfig \
         .hgrc \
         .mplayer \
         .screenrc \
         .tmux.conf \
         .tmux-new-pane_current_path.conf \
         .tmux-new-vi-copy.conf \
         .tmux-old-pane_current_path.conf \
         .tmux-old-vi-copy.conf \
         .tmux-tpm.conf \
         .vimrc \
         .vim \
         .wgetrc \
         .Xdefaults \
         .zsh \
         .zshrc; do
    if [ ! -e "${REPO}/config/$i" ]; then
        echo "Error: $i not found in working directory." >&2
        exit 1
    fi
    if [ ! -e "${HOME}/$i" ]; then
        ln -s "${REPO}/config/$i" "${HOME}/$i"
    elif [ ! -L "${HOME}/$i" ]; then
        echo "Warning: Skipping $i that already exists." >&2
    fi
done

# Link SSH config.
if [ ! -e "${HOME}/.ssh/config_mattutils" ]; then
    ln -s "${REPO}/config/.ssh/config" "${HOME}/.ssh/config_mattutils"
elif [ ! -L "${HOME}/.ssh/config_mattutils" ]; then
    echo "Warning: Skipping SSH config_mattutils that already exists." >&2
fi

# Build some utils.
if [ ! -e "${HOME}/bin/rerebase" ]; then
    cc -O3 -std=c11 -W -Wall -Wextra -Wwrite-strings -Wshadow -Wmissing-declarations -o "${HOME}/bin/rerebase" "${REPO}/rerebase.c"
fi
if [ ! -e "${HOME}/bin/dif" ]; then
    c++ -O3 -std=c++11 -W -Wall -Wextra -Wwrite-strings -Wshadow -Wmissing-declarations -o "${HOME}/bin/dif" "${REPO}/dif.cpp"
fi

if [ ! -e "${HOME}/.tmux/plugins/tpm" ]; then
    echo "Warning: Tmux Plugin Manager not found (~/.tmux/plugins/tpm)" >&2
fi
